...
class MedicalExportRepository {
    async findPrescriptionsForExport(filters, conn = pool) {
        const sql = `
            ...
            WHERE 1=1 ${filters.doctorFilterPr} ${filters.dateFilterPr})
            UNION ALL
            (SELECT 'request' as source_type, r.id, r.request_note as medications, r.doctor_note as instructions, r.completed_at as date,
                    d.full_name as doctor_name, p.full_name as patient_name, p.dni as patient_dni,
            ...
            WHERE r.status = 'completed' ${filters.doctorFilterReq} ${filters.dateFilterReq})
            ORDER BY date DESC
        `;
    ...
    async findGenericRequestsForExport(type, filters, conn = pool) {
        const sql = `
            SELECT r.id, ${type === 'license' ? 'r.request_note as diagnosis, r.doctor_note as days_duration' : 'r.type as certificate_type, r.request_note as description, r.doctor_note as additional_notes'},
            ...
            JOIN doctors d ON r.doctor_id = d.id
            JOIN patients p ON r.patient_id = p.id
            WHERE r.status = 'completed' AND r.type = ? ${filters.doctorFilterReq} ${filters.dateFilterReq}
